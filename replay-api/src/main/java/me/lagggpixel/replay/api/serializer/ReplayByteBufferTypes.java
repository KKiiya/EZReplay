package me.lagggpixel.replay.api.serializer;
...
public final class ReplayByteBufferTypes {
    ...
    public static final ReplayByteBuffer.Type<ItemStack> ITEMSTACK = new ReplayByteBuffer.Type<>() {
        ...
        public ReplayByteBuffer.TypeWriter<ItemStack> writer() {
            return (buffer, itemStack) -> {
                try {
                    ByteArrayOutputStream out = new ByteArrayOutputStream();
                    BukkitObjectOutputStream oout = new BukkitObjectOutputStream(out);
                    oout.writeObject(itemStack);
                    oout.close();

                    byte[] data = out.toByteArray();
                    buffer.write(ReplayByteBuffer.BYTE_ARRAY, data);
                    return data.length;
                } catch (Exception e) {
        ...
        @Override
        public ReplayByteBuffer.TypeReader<ItemStack> reader() {
            return buffer -> {
                try {
                    byte[] data = buffer.read(ReplayByteBuffer.BYTE_ARRAY);

                    ByteArrayInputStream in = new ByteArrayInputStream(data);
                    BukkitObjectInputStream oin = new BukkitObjectInputStream(in);

                    return (ItemStack) oin.readObject();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            };
